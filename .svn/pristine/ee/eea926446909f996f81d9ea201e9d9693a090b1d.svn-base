...
class CreatePrescriptionView(LoginRequiredMixin, DoctorMixin, CreateView):
    ...
    def form_invalid(self, form):
        ...
        form.data['doctor'] = self.request.user
        form.data['patient'] = self.kwargs.get('pk')
        return super(CreatePrescriptionView, self).form_invalid(form)
...
class CreateTestView(DoctorMixin, CreateView):
    ...
    def form_invalid(self, form):
        form.data = form.data.copy()
        form.data['subject'] = self.kwargs.get('pk')
...
class StatisticsJson(LoginRequiredMixin, AdminMixin, View):
     ...
     def get(self, request, *args, **kwargs):
        ...
        if not hospital:
            return JsonResponse(
                {
                'Error': 'You must select a hospital to run analytics. Please go to your profile and select a hospital.'
                }
            )
        return JsonResponse(
            {
                ...
                'stats' :
                {
                    'Average Patients/Day': hospital.average_patient_per_day(),
                    'Average Visits/Patient': hospital.average_visits(),
                    'Average Duration': hospital.average_duration(),
                    'Most Common Reason': hospital.most_common_reason(),
                    'Average # Prescription': hospital.average_prescriptions(),
                }
            }
        )
