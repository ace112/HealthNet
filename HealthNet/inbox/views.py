...
class ReplyMessageView(CreateView):
    ...
    def form_invalid(self, form):
        ...
        form.data['sender'] = self.request.user
        form.data['receiver'] = self.kwargs.get('pk')
        return super(ReplyMessageView, self).form_invalid(form)

    def form_valid(self, form):
        Log.objects.create(log_type='Message Sent', description="Message was sent to: " +
                                                                form.cleaned_data['receiver'].email + ", from:" +
                                                                form.cleaned_data['sender'].email)
...
class SendMessageView(CreateView):
    ...
    def form_invalid(self, form):
        form.data = form.data.copy()
        form.data['sender'] = self.request.user
        return super(SendMessageView, self).form_invalid(form)
